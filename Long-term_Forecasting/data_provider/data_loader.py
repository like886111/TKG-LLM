...
class Dataset_ETT_hour_decomposed(Dataset):
    ...
    def stl_decomposition(self, series, period, variate_name, start_timestamp, end_timestamp):
        ...
        try:
            model = ARIMA(series, order=(1, 1, 0), seasonal_order=(0, 1, 1, period))
            result = model.fit()
            
            trend = pd.Series(result.fittedvalues, index=series.index)
            
            combined = series - trend
            ...
            if n_periods > 0:
                ...
                for i in range(n_periods):
                    ...
                    if end_idx <= len(values):
                        seasonal_pattern += values[start_idx:end_idx]
                seasonal_pattern /= n_periods
                
                repetitions = len(values) // period + (1 if len(values) % period > 0 else 0)
                seasonal_values = np.tile(seasonal_pattern, repetitions)[:len(values)]
                seasonal = pd.Series(seasonal_values, index=series.index)
            else:
                seasonal = pd.Series(np.zeros_like(values), index=series.index)
            
            resid = series - trend - seasonal
        except:
            trend = series.rolling(window=period, min_periods=1, center=True).mean()
            seasonal = pd.Series(np.zeros_like(series.values), index=series.index)
            resid = series - trend
